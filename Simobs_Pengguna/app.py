...
EMAIL_ADDRESS = "muliapratama603@gmail.com" 
EMAIL_PASSWORD = "ljkr ucrj wnlk qoqp" 
...
def do_booking():
    ...
    # Admin akan otomatis membaca booking ini via fungsi sinkron_booking_pengguna()
    # menggunakan field user_booking_id sebagai referensi
...
# =========================
# HELPER: SYNC STATUS DARI ADMIN
# =========================

def sync_status_from_admin(booking_id, current_data):
    """
    Cek status terbaru dari db_admin koleksi 'booking' (singular).
    Admin menyimpan referensi ke booking pengguna via field 'user_booking_id'.
    Jika status berbeda, update juga di db pengguna agar sinkron.
    """
    try:
        from google.cloud.firestore_v1.base_query import FieldFilter
        # Cari di koleksi 'booking' db_admin berdasarkan field user_booking_id
        admin_docs = db_admin.collection("booking").where(
            filter=FieldFilter("user_booking_id", "==", booking_id)
        ).limit(1).stream()

        admin_list = list(admin_docs)
        if admin_list:
            admin_data = admin_list[0].to_dict()
            new_status = admin_data.get("status")
            if new_status and new_status != current_data.get("status"):
                # Update status di db pengguna agar sinkron
                db.collection("bookings").document(booking_id).update({"status": new_status})
                current_data["status"] = new_status
                print(f"[SYNC] Status booking {booking_id} diperbarui: {new_status}")
            # Ambil field tambahan dari admin jika ada
            if admin_data.get("mekanik"):
                current_data["mekanik"] = admin_data.get("mekanik")

    except Exception as e:
        print(f"[sync_status_from_admin] Error untuk booking {booking_id}: {e}")
...
def status_booking():
    ...
    for booking in bookings:
        temp = booking.to_dict()
        temp["id"] = booking.id
        # Sinkronkan status terbaru dari db admin
        temp = sync_status_from_admin(booking.id, temp)
        data.append(temp)
...
def detail_servis(booking_id):
    ...
    if doc.exists:
        ...
        # 4. Sinkronkan status terbaru dari db admin
        booking_data = sync_status_from_admin(booking_id, booking_data)
            
        # 5. Lempar datanya ke detail.html
